...
2. weather 的 action_input 只填纯净的城市名即可（如"北京"，不要包含"明天""预报"等词）
3. 如果用户问"北京明天天气"，action_input 应该是"北京"，不是"北京明天"
...
if __name__ == "__main__":
    ...
    print_header("LangGraph 自动演示 Demo - 智能任务助手")
    print_colored("\n这个 Demo 会自动执行 3 个预定义的任务，展示 LangGraph 的：", Color.WHITE)
    ...
    for i, task in enumerate(tasks, 1):
        print_header(f"任务 {i}/{len(tasks)}: {task}")
        
        result = assistant.run_task(task, max_iterations=3, thread_id=f"task_{i}")
        
        print_header(f"任务 {i} 执行完毕")
        print_colored(f"任务描述: {result['task_description']}", Color.WHITE)
        print_colored(f"任务状态: {result['task_status']}", Color.BRIGHT_MAGENTA)
        print_colored(f"迭代次数: {result['current_iteration']}/{result['max_iterations']}", Color.BRIGHT_CYAN)
        print_colored(f"最终答案: {result['final_answer']}", Color.BOLD + Color.BRIGHT_GREEN)
    
    print_header("所有任务完成！")
    print_success("🎉 所有预定义任务已成功执行完毕！")
